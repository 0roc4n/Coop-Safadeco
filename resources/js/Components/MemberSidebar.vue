...
import { Home, Wallet, Layers3, Landmark, FileText, ReceiptText, ClipboardList, BarChart2, PieChart, Bell, ChevronLeft, ChevronRight, Sun, Moon } from 'lucide-vue-next'
...
const sections = [
	{
		heading: 'Dashboard',
		items: [
			{ label: 'Dashboard Overview', icon: Home, href: route('member.dashboard'), key: 'dashboard' },
		],
	},
	{
		heading: 'My Accounts',
		items: [
			{ label: 'Savings Accounts', icon: Wallet, href: route('member.dashboard'), key: 'savings' },
			{ label: 'Time Deposits', icon: Layers3, href: route('member.dashboard'), key: 'time' },
			{ label: 'Share Capital', icon: Landmark, href: route('member.dashboard'), key: 'share' },
		],
	},
	{
		heading: 'My Loans',
		items: [
			{ label: 'Active Loans', icon: FileText, href: route('member.dashboard'), key: 'loans' },
			{ label: 'Payment History', icon: ReceiptText, href: route('member.dashboard'), key: 'payments' },
			{ label: 'Loan Applications', icon: ClipboardList, href: route('member.dashboard'), key: 'applications' },
		],
	},
	{
		heading: 'Reports',
		items: [
			{ label: 'Statement of Account', icon: FileText, href: route('member.dashboard'), key: 'rpt-soa' },
			{ label: 'Amortization Schedule', icon: BarChart2, href: route('member.dashboard'), key: 'rpt-amort' },
			{ label: 'Dividends & Patronage Refund', icon: PieChart, href: route('member.dashboard'), key: 'rpt-dividends' },
		],
	},
	{
		heading: 'Notifications',
		items: [
			{ label: 'Alerts', icon: Bell, href: route('member.dashboard'), key: 'alerts' },
		],
	},
]
...
<template>
	...
	>
		<div class="flex items-center justify-between px-3 py-3 border-b border-gray-200 dark:border-gray-800">
			<div class="text-sm font-semibold text-gray-700 dark:text-gray-200" v-if="!collapsed">Member</div>
